...
class LastBindingOperationTest(BrokerTestCase):
    ...
    def test_last_operation_called_just_with_required_fields(self):
        self.broker.last_binding_operation.return_value = LastOperation(
        ...
        self.broker.last_binding_operation.assert_called_once_with(
            "here-instance_id", "binding_id", None, None, None
        )
    ...
    def test_last_operation_called_with_operation_data(self):
        ...
        self.client.get(
            "/v2/service_instances/here-instance_id/service_bindings/binding_id/last_operation?%s"
            % query,
            headers={"X-Broker-Api-Version": "2.13", "Authorization": self.auth_header},
        )
    ...
    def test_returns_200_with_given_state(self):
        ...
        query = (
            "service_id=123&plan_id=456&operation=service-guid-here%20operation-data"
        )
        ...
        self.assertEqual(
            response.json,
            dict(state=OperationState.IN_PROGRESS.value, description="Running..."),
        )
