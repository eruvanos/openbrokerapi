...
class FullRouterTestCase(TestCase):
    ...
    def test_lifecycle(self):
        ...
        # PROVISION
        self.check_provision(instace_guid, org_guid, space_guid, self.broker_1.service_guid, self.broker_1.plan_guid)
    ...
    def check_instance_retrievable(self, instace_guid):
        response = requests.get(
            "http://localhost:5002/v2/service_instances/{}".format(instace_guid), **self.request_ads)
    ...
    def check_unbind(self, binding_guid, instace_guid):
        response = requests.delete(
            "http://localhost:5002/v2/service_instances/{}/service_bindings/{}".format(instace_guid, binding_guid),
            params={
    ...
    def check_provision(self, instace_guid, org_guid, space_guid, service_guid, plan_guid):
        response = requests.put(
            "http://localhost:5002/v2/service_instances/{}?accepts_incomplete=false".format(instace_guid),
            data=json.dumps({
